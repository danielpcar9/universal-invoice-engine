...
class PeppolParser:
    ...
    def parse(cls, xml_bytes: bytes) -> ParsedInvoice:
        ...
        try:
            ...
            customer_vat = xpath_text(
                "//cac:AccountingCustomerParty/cac:Party/cac:PartyTaxScheme/cbc:CompanyID/text()"
            )

            # 5. Extract Monetary Totals (LegalMonetaryTotal)
            payable_str = xpath_text(
                "//cac:LegalMonetaryTotal/cbc:PayableAmount/text()"
            )
            tax_exclusive_str = xpath_text(
                "//cac:LegalMonetaryTotal/cbc:TaxExclusiveAmount/text()"
            )
            tax_inclusive_str = xpath_text(
            ...
            subtotal = (
                Decimal(tax_exclusive_str) if tax_exclusive_str else Decimal("0.00")
            )
            ...
            logger.info(
                f"Successfully parsed PEPPOL Invoice {invoice_id} from {supplier_name}"
            )
